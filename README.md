...
- 🔬 **글자별 오타 리뷰** — 결과 화면에서 지문을 글자 단위로 펼쳐, 각 위치마다 **지문 글자(`target[i]`)와 친 글자(`typed[i]`)를 1:1 비교**해 보여줍니다. 맞은 글자는 **에메랄드**, 틀린 글자는 **로즈**로 표시하되 지문 글자에 취소선을 긋고 그 아래에 **실제로 친 글자**를 작게 적어 "여기서 X를 Y로 잘못 쳤다"가 바로 보입니다. 엔터로 중간에 끝낸 나머지는 흐릿한 **미입력**으로 구분합니다. 하단에는 `안 → 아` 형태의 **오타 요약 목록**도 함께 제공합니다. (채점과 동일하게 코드포인트 단위로 매칭, 공백은 `␣`·빈 입력은 `∅`로 표기)
