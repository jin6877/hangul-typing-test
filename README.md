...
- ⏎ **엔터로 완료 / 다음** — 한 글자라도 친 상태라면 `Enter`로 측정을 즉시 종료합니다. 오타가 있거나 지문을 끝까지 치지 않았어도, 그 시점까지 친 분량을 기준으로 결과(타수·정확도)가 확정됩니다. 결과 화면에서 한 번 더 누르면 다음 문장으로 넘어갑니다. 한글 IME 조합 중인 엔터는 결과 확정과 혼동되지 않게 무시합니다.
